...
import { X, Check, Crown, Sparkles, Zap, Star } from "lucide-react";
import { subscriptionPlans, SubscriptionPlan } from "@/lib/chatData";
...
const planIcons: Record<string, React.ReactNode> = {
  guest: <Zap className="w-5 h-5" />,
  basic: <Star className="w-5 h-5" />,
  advanced: <Sparkles className="w-5 h-5" />,
  pro: <Crown className="w-5 h-5" />,
};

export default function SubscriptionModal({ isOpen, onClose, currentPlan, onSelectPlan }: SubscriptionModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          <motion.div
            ...
            className="w-full max-w-4xl glass-heavy border border-border rounded-2xl float-shadow overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-thin"
          >
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {subscriptionPlans.map((plan, i) => (
                ...
                >
                  ...
                  >
                    {currentPlan === plan.id ? "Current Plan" : plan.price === "Free" ? "Continue as Guest" : "Upgrade"}
                  </button>
                </motion.div>
              ))}
            </div>
